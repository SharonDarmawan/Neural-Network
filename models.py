    ...
    def train(self, dataset):
        ...
        while not converged:
            ...
            for x, y in dataset.iterate_once(batch_size):
                ...
                # If data is misclassified, update the weights and loop over dataset again
                if multiplier != self.get_prediction(x):
                    parameter.update(x, multiplier)
                    converged = False
...
class LanguageIDModel(object):
    ...
    def __init__(self):
        ...
        self.w2 = nn.Parameter(300, len(self.languages))
        self.b2 = nn.Parameter(1, len(self.languages))
        self.w_hidden = nn.Parameter(300, 300)
        self.b_hidden = nn.Parameter(1, 300)

        self.parameters = [self.w1, self.b1, self.w2, self.b2, self.w_hidden, self.b_hidden]
    ...
    def run(self, xs):
        ...
        for x in xs[1:]:
            z = nn.Add(nn.Linear(x, self.w1), nn.Linear(h, self.w_hidden))
            h = nn.ReLU(nn.AddBias(z, self.b_hidden))
        h = nn.AddBias(nn.Linear(h, self.w2), self.b2)
        return h
